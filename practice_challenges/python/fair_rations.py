def fairRations(queue) -> str:
    """Calculate number of counts needed for an even distribution"""
    count = 0
    index = 0
    ...
    while index < len(queue) - 1:
        ...
        is_next_even = next_val % 2 == 0

        if index == len(queue) - 2:
            if (is_current_odd and is_next_even) or (
                not is_current_odd and not is_next_even
            ):
                return "NO"
